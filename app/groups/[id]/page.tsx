...
export default function GroupProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!group) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-slate-950 text-slate-900 dark:text-white">
      ...
      <Button variant="ghost" asChild><Link href="/groups">Вернуться к списку</Link></Button>
    </div>
  );
      ...
      <div className="flex items-center justify-between bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-slate-800">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild className="rounded-full">
            <Link href="/dashboards"> {/* Или /groups, смотря откуда пришли */}
              <ArrowLeft size={20} />
            </Link>
          </Button>
        ...
        <div className="lg:col-span-9 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {isComputing ? (
               ...
               return (
                 <Card key={metric.virtualMetricId} className="p-5 flex flex-col justify-between hover:border-indigo-300 dark:hover:border-indigo-700 transition-all group">
                    ...
                    <div className="mt-2">
                       ...
                       {/* Можно добавить дельту/тренд если есть данные */}
                    </div>
                 </Card>
               );
            })}

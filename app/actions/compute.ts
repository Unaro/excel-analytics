...
import { ActiveHierarchyFilter } from '@/types/computed';
...
export async function computeDashboardMetrics(params: {
  data: ExcelRow[];
  allGroups: IndicatorGroup[];           // Все доступные определения групп (с формулами)
  dashboardGroupsConfig: IndicatorGroupInDashboard[]; // Конфигурация групп на дашборде (с привязками)
...
async function computeGroupMetrics(
  ...
  for (const metric of sortedMetrics) {
    ...
    try {
      const value = await computeMetric(metric, template, data, metricValues, groupDef);
...
async function computeMetric(
  ...
  if (template.type === 'aggregate') {
    return computeAggregateMetric(metric, template, data, group);
  } else {
    return computeCalculatedMetric(metric, template, data, metricValues, group);
  }
}
...
function computeCalculatedMetric(
  ...
  try {
    // Создаем scope для вычисления формулы
    ...
    for (const metricBinding of metric.metricBindings) {
      ...
      scope[metricBinding.metricAlias] = value ?? null;
    }
    
    // Проверяем, что все зависимости разрешены
    // (В mathjs null может вызвать ошибку в некоторых операциях, лучше обработать)
    const hasNullDependency = Object.values(scope).some(v => v === null);
    
    // В градостроительстве часто null = 0 для суммирования, но для деления это критично.
    // Оставим строгую проверку пока что.
    if (hasNullDependency) {
      return null;
    }

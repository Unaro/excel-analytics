...
## Статус исправлений (2026-06-11)

Исправлено в фазе 6 (см. [refactoring/06-core-audit-fixes.md](refactoring/06-core-audit-fixes.md)):
**№1, №2, №3, №4, №5, №6, №7, №8, №10** — группы 1–3 порядка работ целиком.
Дополнительно закрыт пользовательский баг: показатель с удалённым шаблоном
нельзя было удалить из группы (MetricRow рендерил `null` вместо строки).

Открыты: №9 (двойной триггер use-computation), №13 (hierarchyFilters в persist —
UX-решение), №15 (линейные find в компиляторе), №16-частично (откат батча
импорта, dispose в cleanup — обе реализации no-op).

## Статус исправлений (2026-06-21)

**№11 (объединение KPI-запроса)** — KPI-группа считается одним проходом вместе
с дашбордом (`use-dashboard-computation` + `kpi-compiler.mapResultsToKPI`);
`use-kpi-calculation` удалён, `KPIGrid` получает значения пропом. Заодно дашборд
стал передавать `validColumns` (обязателен для PG). **№12** — кэш DESCRIBE
(`schemaCache`). **№14** — deps-разнобой устранён (`widgets.length` ушёл с
удалённым хуком; `use-group-breakdown` на хешах). **№16** — `immer` удалён из
зависимостей (нигде не импортировался).

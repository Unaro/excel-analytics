...
№13 (hierarchyFilters в persist) — закрыт как **won't fix** (2026-06-21):
сохранение фильтров между перезагрузками — намеренное поведение (состояние
просмотра), не баг.

Открыты: — (все хвосты ядра закрыты, см. ниже).

## Статус исправлений (2026-06-21, часть 2)

- **№9** (двойной триггер use-computation) — закрыт: `buildParams`/`buildCacheKey`
  уже живут в ref'ах, `executeInternal` их не держит в deps, `execute` стабилен
  на контентных изменениях; контракт зафиксирован в JSDoc хука. Двойного запуска
  на смену фильтра нет.
- **№15** (линейные find в компиляторе) — `compileQuery` строит `groupById` /
  `templateById` (`Map`) до вложенного цикла вместо `.find()` на каждой метрике
  (было O(групп × метрик × шаблонов)).
- **№16** — откат батча импорта: при ошибке INSERT в batched-стратегии
  (`worker.ts`) сбрасываются temp- и частично заполненная целевая таблица
  (`try/catch` + `DROP`), чтобы COUNT(*) не принял «полуимпорт» за успех;
  `IComputeEngine.dispose(datasetId)` теперь вызывается в cleanup
  `use-computation` (обе реализации пока no-op — фиксация контракта).

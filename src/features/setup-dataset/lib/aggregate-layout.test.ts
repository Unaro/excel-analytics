...
describe('toAggregateCsv: канонизация метрик, экранирование', () => {
  ...
  it('нечисловой токен в метрике → пусто (колонка не уходит в VARCHAR)', () => {
    // Колонка числовая (>0.6 ячеек — числа) → role=metric; одиночный текст «н/д»
    // должен стать пустым, иначе DuckDB типизирует колонку VARCHAR.
    const header = [['Регион', 'Потребность']];
    const rows = [['А', '12,5'], ['Б', '100'], ['В', '250,7'], ['Г', '1 000'], ['Д', 'н/д']];
    const res = flattenLeaves([...header, ...rows], { headerRows: 1, keyColumns: [0] });
    const lines = toAggregateCsv(res).split('\n');
    expect(lines[1]).toBe('А,12.5');
    expect(lines[3]).toBe('В,250.7');
    expect(lines[4]).toBe('Г,1000');
    expect(lines[5]).toBe('Д,');   // «н/д» → пусто (NULL), не текст
  });
...
describe('parseMetricValue: число или null', () => {
  it('RU-число → number', () => {
    expect(parseMetricValue('1 639 132')).toBe(1639132);
    expect(parseMetricValue('0,144')).toBeCloseTo(0.144);
    expect(parseMetricValue('0')).toBe(0);
  });
  it('пусто/нечисло → null', () => {
    expect(parseMetricValue('')).toBeNull();
    expect(parseMetricValue('—', { tokens: ['—'] })).toBeNull();
    expect(parseMetricValue('abc')).toBeNull();
  });
});

describe('extractNodes: узлы + введённые значения (фаза 2)', () => {
  const M: string[][] = [
    ['Регион', 'Город', 'Зона', 'Объект', 'Площадь', 'Численность'], // шапка
    ['Тюменская обл', '', '', '', '', '1639132'],                    // узел level 0
    ['Тюменская обл', 'Тюмень', '8:01:06', '', '0,144', ''],          // зона-узел level 2
    ['Тюменская обл', 'Тюмень', '8:01:06', '{guid1}', '1,45', '0'],   // лист
    ['Итого', '', '', '', '', '1727833'],                            // «Итого»
  ];
  const nodes = extractNodes(M, { headerRows: 1, keyColumns: [0, 1, 2, 3] });

  it('берёт узлы/итого, листья пропускает', () => {
    expect(nodes.every(n => n.label !== '{guid1}')).toBe(true);
    expect(nodes.some(n => n.isTotal)).toBe(true);
  });
  it('узел зоны несёт свой путь и введённое значение метрики', () => {
    const zone = nodes.find(n => n.label === '8:01:06');
    expect(zone?.path).toEqual(['Тюменская обл', 'Тюмень', '8:01:06']);
    expect(zone?.values['Площадь']).toBeCloseTo(0.144);
    expect(zone?.values['Численность']).toBeNull();
  });
  it('узел города несёт введённую Численность', () => {
    const city = nodes.find(n => n.path.length === 1 && n.label === 'Тюменская обл' && !n.isTotal);
    expect(city?.values['Численность']).toBe(1639132);
  });
  it('nodePathKey стабилен и не схлопывает пути', () => {
    expect(nodePathKey(['a', 'b'])).toBe(nodePathKey(['a', 'b']));
    expect(nodePathKey(['a', 'b'])).not.toBe(nodePathKey(['ab']));
  });
});

...
import { ChartConfig, ChartType, DataScope } from '@/types/dashboard-builder';
import { 
  X, 
  Save, 
  BarChart3, 
...
export default function ChartEditor({
  ...
  const handleSave = () => {
    ...
    const finalConfig: ChartConfig = {
      ...
      dataScope: editConfig.dataScope || 'hierarchy',
      x: editConfig.x || 0,
      y: editConfig.y || 0,
      w: editConfig.w || 6,
      h: editConfig.h || 4,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] overflow-hidden">
                    ...
                    >
                      ...
                      <div>
                        <div className={`font-medium ${
                          editConfig.type === type.type ? 'text-blue-900' : 'text-gray-700'
                        }`}>
                          {type.label}
                        </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditConfig({ ...editConfig, w: parseInt(e.target.value) || 6 })}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setEditConfig({ ...editConfig, h: parseInt(e.target.value) || 4 })}
                      className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
                <div className="mt-3 text-xs text-gray-500">
                  Предпросмотр: {editConfig.w || 6} колонок × {editConfig.h || 4} строк
                </div>
              </Card>
            </div>
                  ...
                  {(['hierarchy', 'global'] as const).map((scope) => (
                    ...
                    >
                      <input
                        ...
                        name="dataScope"
                        value={scope}
                        checked={editConfig.dataScope === scope}
                        onChange={(e) => setEditConfig({ ...editConfig, dataScope: e.target.value as DataScope })}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`font-medium ${
                            ...
                            {scope === 'hierarchy' ? 'Иерархические фильтры' : 'Глобальная область'}
                          </span>
                        </div>
                        <div className="text-xs text-gray-500 mt-1">
                          {scope === 'hierarchy' 
                            ? 'Учитывать выбранный путь в иерархии' 
                            : 'Использовать все данные, игнорируя иерархические фильтры'
                          }
                        </div>
                      </div>
                    </label>
              ...
              <Card title="Источник данных">
                <div className="grid grid-cols-3 gap-3">
                  {(['groups', 'raw', 'sql'] as const).map((source) => (
                    <button
                      key={source}
                      onClick={() => setEditConfig({ ...editConfig, dataSource: source, groupIds: [] })}
                          ...
                          <label
                            ...
                            className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${
                              editConfig.groupIds?.includes(group.id)
                                ...
                                : 'bg-gray-50 border-2 border-gray-200 hover:bg-blue-50'
                            }`}
                          >
                            <input
                              type="checkbox"
                  ...
                  >
                    {availableIndicators.length > 0 ? (
                          ...
                          <label
                            ...
                            className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors ${
                              editConfig.indicators?.includes(indicator)
                                ...
                                : 'bg-gray-50 border-2 border-gray-200 hover:bg-green-50'
                            }`}
                          >
                            <input
                              type="checkbox"
                      ...
                      <div className="text-center py-6 text-gray-500">
                        ...
                        <p className="text-sm">
                          {editConfig.groupIds && editConfig.groupIds.length > 0
                            ? 'Нет общих показателей в выбранных группах'
                            : 'Выберите группы для получения показателей'
                          }
                        </p>
                      </div>
                    )}
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-purple-900">
                      {editConfig.dataScope === 'hierarchy' ? 'Иерархическая' : 'Глобальная'}
                    </span>
                  </div>
                  {editConfig.dataSource === 'groups' && (
                    <>
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{editConfig.groupIds?.length || 0}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Показателей:</span>
                        <span className="font-medium">{editConfig.indicators?.length || 0}</span>
                      </div>
                    </>
                  )}
                </div>
        ...
        <div className="sticky bottom-0 bg-gray-50 px-6 py-4 flex gap-3 border-t border-gray-200">
          <button
            onClick={handleSave}
            disabled={!editConfig.title || !editConfig.type}
            className="flex-1 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg font-bold hover:from-blue-700 hover:to-purple-700 disabled:from-gray-400 disabled:to-gray-500 transition-all shadow-lg disabled:cursor-not-allowed"
          >

...
export function HierarchicalFilter({
  ...
  // Вычисляем данные для каждого уровня иерархии
  const levelsData = useMemo((): LevelData[] => {
    ...
    for (let i = 0; i < hierarchyConfig.length; i++) {
      ...
      // Если есть выбранное значение, фильтруем данные для следующего уровня
      if (selectedValue) {
        currentData = currentData.filter(row => String(row[level]) === selectedValue);
      } else {
        // Если нет выбранного значения на текущем уровне, прекращаем построение дерева
        break;
      }
    }
  ...
  // Получаем статистику для текущих фильтров
  ...
  // Переключение развернутости уровня
  ...
  // Выбор значения на уровне
  const selectValue = useCallback((level: string, value: string | null) => {
    const newFilters = { ...currentFilters };
    
    if (value === null) {
      // Сбрасываем текущий уровень и все дочерние
      const levelIndex = hierarchyConfig.indexOf(level);
      hierarchyConfig.slice(levelIndex).forEach(l => {
        delete newFilters[l];
      });
    } else {
      // Устанавливаем значение и сбрасываем все дочерние уровни
      newFilters[level] = value;
      const levelIndex = hierarchyConfig.indexOf(level);
      hierarchyConfig.slice(levelIndex + 1).forEach(l => {
        delete newFilters[l];
      });
    }
    ...
    return (
      <Card className={className}>
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm">
            Иерархическая структура не настроена.
            <br />
            Перейдите в настройки для конфигурации.
          </p>
        </div>
      </Card>
    );
  ...
  return (
    ...
    >
      <div className="space-y-1">
        {/* Информация о текущей выборке */}
        {hasActiveFilters && (
          <div className="mb-4 p-3 bg-blue-50 rounded-lg">
            <div className="flex items-center justify-between text-sm">
              <span className="text-blue-800 font-medium">
                Отфильтровано: {filteredDataCount.toLocaleString()} из {data.length.toLocaleString()} строк
              </span>
              <span className="text-blue-600 text-xs">
                {Math.round((filteredDataCount / data.length) * 100)}%
              </span>
            </div>
          </div>
        )}
              ...
              <div key={levelData.level}>
                ...
                >
                  ...
                  <span className={`font-medium ${
                    hasSelection ? 'text-blue-600' : 'text-gray-700'
                  }`}>
                    {levelData.level}
                  </span>
                  ...
                  {hasSelection && (
                    <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
                      {levelData.selectedValue}
                    </span>
                  )}
                  
                  <span className="text-xs text-gray-500">
                    ({levelData.values.length} вариантов)
                  </span>
                  ...
                  {hasSelection && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        selectValue(levelData.level, null);
                      }}
                ...
                {/* Сообщение о пустом уровне */}
                {isExpanded && levelData.values.length === 0 && (
                  <div 
                    className="py-2 px-3 text-xs text-gray-500 italic"
                    style={{ paddingLeft: `${28 + indentLevel * 16}px` }}
                  >

...
describe('compileQuery: aggregate-метрики', () => {
  ...
  it('PostgreSQL: каст через NULLIF(...)::DOUBLE PRECISION', () => {
    const { sql, params } = compileQuery(makeParams(), 'postgres');
  ...
  it('MEDIAN: MEDIAN() в DuckDB, PERCENTILE_CONT в PostgreSQL', () => {
    const params = makeParams({
      metricTemplates: [makeAggregateTemplate({ aggregateFunction: 'MEDIAN' })],
    });

    expect(compileQuery(params, 'duckdb').sql).toContain('MEDIAN(');
    expect(compileQuery(params, 'postgres').sql).toContain(
...
describe('compileQuery: валидация колонок и идентификаторов', () => {
  // ВНИМАНИЕ (Phase 2 security): текущее поведение НАМЕРЕННО зафиксировано
  // здесь как небезопасное. При validColumns === undefined компилятор
  // доверяет любым именам колонок, а tableName интерполируется в SQL как есть.
  // Для PostgreSQL-диалекта Phase 2 делает validColumns обязательным
  // (whitelist с сервера из information_schema) — эти тесты будут обновлены
  // вместе с этим изменением.
  it('без validColumns любая колонка считается допустимой (текущее поведение)', () => {
    const params = makeParams({
  ...
  it('tableName интерполируется без проверки (текущее поведение)', () => {

...
export interface VirtualMetricValue {
  ...
  fromNode?: boolean;
  /**
   * Формат, в масштабе которого красить/сравнивать с порогами это значение,
   * если он отличается от собственного формата метрики. Ставит `normalizeVmRows`
   * для нормализованных значений: `value` становится долей, показываемой
   * процентом, поэтому окрашивание тоже должно идти в шкале `percent` (иначе
   * 1.51 = «между 0 и 100», хотя на экране 151%). Строки мимо нормализации
   * (напр. «Итого») поле не получают и красятся в своём формате.
   */
  colorFormat?: string;
}

...
export const IndicatorGroupSchema = z.object({
  ...
  displayFilters: z.array(z.object({
    ...
    /**
     * Глубина уровня иерархии, на котором действует правило (= длина пути).
     * Правило применяется ТОЛЬКО когда пользователь на этом уровне — фильтр не
     * распространяется на дрилл вглубь (иначе родитель, не прошедший правило,
     * скрывался бы и в него нельзя было провалиться). Нет значения (старые
     * правила) → 0 (корень).
     */

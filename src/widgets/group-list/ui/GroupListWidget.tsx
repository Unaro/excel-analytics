      ...
      <div className="p-8 max-w-6xl mx-auto space-y-8">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">
              Логические блоки (строки таблицы)
            </p>
          </div>
          <Button
        ...
        <div className="grid gap-4">
          ...
          {groups.map(group => (
            <Card
              key={group.id}
              className="p-5 flex items-center justify-between hover:border-emerald-400/50 transition-colors group"
            >
                ...
                <div>
                  ...
                  {group.datasetId && (
                    <span className="inline-flex items-center gap-1 mt-1 text-[10px] text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded w-fit">
                      <Database size={10} />{' '}
                      {datasets[group.datasetId]?.name || 'Нет привязки'}
                    </span>
                  )}
                </div>
              </div>
